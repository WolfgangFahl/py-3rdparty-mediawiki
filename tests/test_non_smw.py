...
class TestNonSMW(BaseWikiTest):
    ...
    def test_category_query(self):
        ...
        for ask_query,limit,expected_category,expected_pages,debug_pages in test_cases:
            category,_mainlabel=wikipush.extract_category_and_mainlabel(ask_query)
            self.assertEqual(expected_category,category)
            pages = wikipush.query(ask_query, limit=limit)  # Limit to 10 for faster testing
